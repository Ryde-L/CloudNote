...
(function(factory) {
	...
	$.fn.ajaxSubmit = function(options) {
		...
		function fileUploadIframe(a) {
			...
			// add_input submitting element to data if we know it
			sub = form.clk;
			if (sub) {
				n = sub.name;
				if (n && !sub.disabled) {
			...
			function doSubmit() {
				...
				// add_input "extra" data to form if provided in options
				var extraInputs = [];
				try {
					if (s.extraData) {
					...
					if (!s.iframeTarget) {
						// add_input iframe to doc and submit the form
						$io.appendTo('body');
					}
					if (io.attachEvent) {
	...
	$.fn.Huispinner = function(options, callback) {
		...
		this.each(function() {
			...
			var str = '<div class="spinner">'
					...
					+ '<a class="add_input" href="javascript:void(0)"><i>+</i></a>'
					+ '</div>';
			that.append(str);

			var input = that.find(".input-text"),
				subtract = that.find(".subtract"),
				add = that.find(".add_input"),
		...
		click: function(score) {
			return $(this).each(function() {
				...
				if (this.opt.click) {
					...
					methods.error.call(this, 'you must add_input the "click: function(score, evt) { }" callback.');
				}
		...
		function defineBridget($) {
			...
			*/
			function bridge(namespace, PluginClass) {
				// add_input to jQuery fn namespace
...
! (function(factory) {
	...
	var Datetimepicker = function(element, options) {
		...
		// add_input container for single page application
		...
		this.component = this.element.is('.date') ? (this.bootcssVer == 3 ? this.element.find('.input-group-addon .glyphicon-th, .input-group-addon .glyphicon-time, .input-group-addon .glyphicon-remove, .input-group-addon .glyphicon-calendar, .input-group-addon .fa-calendar, .input-group-addon .fa-clock-o').parent() : this.element.find('.add_input-on .icon-th, .add_input-on .icon-time, .add_input-on .icon-calendar, .add_input-on .fa-calendar, .add_input-on .fa-clock-o').parent()) : false;
		this.componentReset = this.element.is('.date') ? (this.bootcssVer == 3 ? this.element.find('.input-group-addon .glyphicon-remove, .input-group-addon .fa-times').parent() : this.element.find('.add_input-on .icon-remove, .add_input-on .fa-times').parent()) : false;
