...
function getCurrentSvgElement(){
  return currentSvgElement;
}

function setCurrentSvgElement(ev,e) {
  if (e) {
    currentSvgElement = e;
    return;
  }

  if (ev.target.tagName === 'path' && ev.target.parentElement.tagName === 'g')
    currentSvgElement = ev.target.parentElement;
  else
    currentSvgElement = ev.target;
}
  ...
  function () {
    ...
    _createClass(DragHandler, [{
      ...
      value: function startDrag(ev) {
        ...
        setCurrentSvgElement(ev,null);
        // console.log("svg.draggable.js开始拖动");
      ...
      value: function drag(ev) {
        // console.log("svg.draggable.js正在拖动");

        var box = this.box,
            lastClick = this.lastClick;
        var currentClick = this.el.point(getCoordsFromEvent(ev));

        var x = Math.ceil(box.x + (currentClick.x - lastClick.x));
        var y = Math.ceil(box.y + (currentClick.y - lastClick.y));
      ...
      value: function move(x, y) {
        // console.log("svg.draggable.js 移动");
      ...
      value: function endDrag(ev) {
        // console.log("svg.draggable.js endDrag");
