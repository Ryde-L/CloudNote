        ...
        try {
            ...
            if (userId == null) {
                ...
                if (request.getHeader("user-ajax") != null)
                    responseError(ctx, "302", "http://localhost:8800/sso/path/login.html", "未登陆");
                else
                    response.sendRedirect("http://localhost:8800/sso/path/login.html");
                return null;
            }
            //对请求进行路由
